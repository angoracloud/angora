...
- **Constants discipline**: pull a value into `src/constants.ts` (or `src/routes.ts`'s `ROUTES`/`ROUTE_SEGMENTS` for paths) instead of hardcoding it inline when it's **(a) duplicated in 2+ places** (e.g. `CURRENT_USER.NAME` — was independently typed as `"Angora Admin"` in both `Sidebar.tsx` and `TopBar.tsx` until a PR review caught it), **(b) config-like** — timing/API paths/external URLs/default IDs/error-message fallbacks, the kind of thing that changes together (e.g. `DEFAULT_ERROR_REASON`, `TIMING_CONFIG`, `DISCORD_CONFIG`), or **(c) something that already has an obvious home** in an existing constants group (a new confirm/toast message belongs next to `CONFIRM_MESSAGES`/`TOAST_MESSAGES`, not inline in a component — this is what the review comment on `ConnectedServersTab.tsx`'s `confirm()` call was about). Two real gaps closed this way: `NOT_FOUND_TITLE` (was hand-duplicated between `NotFoundPage.tsx`'s heading and `TopBar.tsx`'s not-found fallback, with a comment explicitly noting they had to be kept in sync by hand) and every `createRoute({ path })` segment in `router.tsx` (was retyped as a raw string instead of reusing `ROUTES`). Don't over-extract, though — one-off UI copy that appears exactly once and isn't config-like (a heading, a button label, a paragraph) stays inline; that's not what this rule is for.
...
- `src/constants.ts` — API endpoints, timing config, confirm/toast message templates, and other config-like or duplicated values (see the constants-discipline bullet above)
