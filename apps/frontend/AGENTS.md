...
- **Constants discipline — no hardcoded strings in components, full stop.** Every literal that renders as UI text or a meaningful prop value (headings, paragraph copy, button/link labels, empty/loading/error-state messages, `placeholder`, `aria-label`, `title` tooltips) belongs in a named constant, not typed inline in JSX — even if it currently appears exactly once. Two files split this by concern:
  - **`src/copy.ts`** — user-facing UI text, one export per screen/component (`HOME_PAGE_COPY`, `DISCORD_PAGE_COPY`, `CONNECTED_SERVERS_COPY`, `SERVER_CARD_COPY`, `SLASH_COMMANDS_COPY`, `BACKEND_HEALTH_COPY`, `SETTINGS_PAGE_COPY`, `NOT_FOUND_COPY`, `SIDEBAR_COPY`, `TOPBAR_COPY`, `TOAST_CONTAINER_COPY`, `NAV_COPY`, `PAGE_TITLES`). A new screen gets its own `<SCREEN>_COPY` export here — group by screen, not one flat namespace.
  - **`src/constants.ts`** — non-copy config: API endpoints, timing, external IDs/URLs, and message *templates* that take an argument (`CONFIRM_MESSAGES`, `TOAST_MESSAGES` — these stay here, not in `copy.ts`, since they're parameterized functions built from runtime data, not static screen copy).
...
- `src/copy.ts` — Static UI text, grouped per screen/component (see the constants-discipline bullet above)
