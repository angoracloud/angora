...
2. **Postgres is the exception.** It's a third-party image and only reads `POSTGRES_PASSWORD` at initdb, before any of our code runs. Sourcing it from Infisical means feeding compose itself, as a **one-time bootstrap against an empty volume** — not the normal run command. Afterwards `POSTGRES_PASSWORD` is ignored entirely and plain `docker-compose up` is correct. `--env-file` makes compose read that file *instead of* `.env`, so merge rather than replace: `cat .env .env.infisical > .env.bootstrap` (later lines win), then `--env-file .env.bootstrap`. Rotating `POSTGRES_PASSWORD` later needs `ALTER USER` or `down -v`, since initdb has already run.

   Only secrets belong in the project. `DB_URL` deliberately has no `POSTGRES_DB` fallback — it's a JDBC string compose builds — so a non-default database name goes in `.env`, not Infisical.
